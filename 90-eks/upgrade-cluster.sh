...
#   if [[ -z "$LATEST" ]]; then
#     echo -e "${R}No version found for $addon${N}"
#     continue
#   fi

#   echo -e "${Y}$addon: $CURRENT -> $LATEST${N}"

#   if [[ "$CURRENT" == "$LATEST" ]]; then
#     echo -e "${G}$addon already latest${N}"
#     continue
#   fi
...
#     echo "$addon status=$STATUS version=$VERSION"

#     if [[ "$STATUS" == "ACTIVE" && "$VERSION" == "$LATEST" ]]; then
#       echo -e "${G}$addon upgraded${N}"
#       break
#     fi

#     if [[ "$STATUS" == "FAILED" || "$STATUS" == "DEGRADED" ]]; then
#       echo -e "${R}$addon failed${N}"
#       exit 1
#     fi
